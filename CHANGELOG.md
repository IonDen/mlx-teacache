...
### Changed
- **mflux 0.19 is supported; the pin widens to `>=0.17.5,<0.20`.** 0.19 adds an optional controlnet hook to the Z-Image transformer forward and gives every generate loop a `bake_lora` flag and a PiD-decoder branch, all off by default, so the forwards this library copies take the same path they took on 0.18; the drift guard carries 0.19.1's fingerprints, and the real-weights parity lane passed on 0.19.1 for FLUX.1-dev, FLUX.1 Krea [dev], the FLUX.2 Klein family and Z-Image (the single schnell test was not run on 0.19.1; it exercises the same FLUX.1 path as the fourteen dev tests). Qwen-Image is the exception: mflux 0.19 resolves the `qwen-image` alias to `Qwen/Qwen-Image-2512`, a newer checkpoint than the `Qwen/Qwen-Image` the coefficients were calibrated on, and that checkpoint was not verified (58 GB of weights and a 26 GB parity peak did not fit the bench machine), so on mflux 0.19 the variant still applies but `apply_teacache` warns that its operating point is unmeasured (see Added). Before upgrading: mflux 0.19.1 pulls in torch and opencv as dependencies, the project now lives at the `mflux-community/mflux` repository (the old URL redirects), and mflux 0.19 requires MLX 0.32, on which Z-Image's lazily built rotary tables make a vanilla run after any eager pass through the transformer differ from one before it by Metal noise (max-abs 6.25e-2, cosine 0.999977 on the 8-step parity recipe, measured on an M1 Max). `restore()` leaves nothing behind; the parity fixture evaluates the tables once after loading, and the Z-Image variant page shows the one-liner if you need the same repeatability. mlx-taef users pair mflux 0.19 with mlx-taef 0.8.1 or later, whose live-preview callback accepts the new `control_images` argument.
