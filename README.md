...
```bash
pip install "mlx-teacache[mflux]==0.10.1"  # pin for reproducibility
```
...
**[`z-image-base`](docs/variants/z-image-base.md)** — Z-Image base (Tongyi-MAI, Apache-2.0), a single-stream DiT and the first non-FLUX model with a TeaCache mini-kernel (added in v0.7.0). Its adaLN modulation is timestep-only, so there is no cheap caption-independent modulation input to gate on; the gate signal is the first-main-layer residual (calibrated in-repo as "Signal B", R² 0.400 — in line with the shipped FLUX.2 fits, and the threshold sweep rather than the fit R² sets the quality bar here; the caption-independent noise-refiner tap was tried and rejected at R² 0.069). Per-variant default `rel_l1_thresh=0.12`, set at the SSIM knee. At the 512×512 red-apple recipe (subprocess-per-rep, q8, 50 steps, g=4.0, v0.10.0 bench): **1.31× combined wall-clock** (227.4 s → 174.2 s; 15/48 active steps skipped in every rep), the win entirely from gating: the no-gate wrapper runs at vanilla speed (0.99×), so `mx.compile`-path avoidance is not a tailwind on Z-Image. Peak memory drops 17.2 GB → 11.5 GB (from the eager wrapper bypassing mflux's compiled `_predict`, not from gating). SSIM 0.991 vs vanilla at this recipe. v0.7.0 reported 1.17× at the same recipe (245.3 s → 209.4 s); the skip count and pattern are identical, so that session's slower wrapper time came from the host (thermals or power), not from the gate. The [COMPARISON.md](COMPARISON.md) portrait row is a separate 640×896 generation.

See `scripts/_bench_z_image_v0_7_0.json` for the full report.

**[`qwen-image`](docs/variants/qwen-image.md)** — Qwen-Image base (Alibaba, Apache-2.0), a ~20B dual-stream MMDiT and the first variant that proxies `flux.transformer` (the FLUX.1 pattern) *and* runs true two-pass CFG. It's FLUX-shaped, so the gate taps the FLUX-canonical modulated block-0 input — calibrated in-repo at R² 0.849, well above Z-Image's 0.400 and the FLUX.2 family's 0.11–0.47. Per-variant default `rel_l1_thresh=0.30`, set from the threshold sweep. The v0.10.0 three-way bench at the red-apple 768×768 recipe (subprocess-per-rep, three cold reps, q4, 50 steps, g=4.0): **3.02× combined** (850.3 s → 282.0 s), 33 of 48 active steps skipped, SSIM 0.967 vs vanilla. Qwen moved further under the corrected gate than any other variant — 0.9.x skipped 24 at this same threshold and held SSIM 0.978 — because its accumulator sits nearest the threshold, so it now runs further along the speed/quality curve; lower `rel_l1_thresh` to walk that back. Against the wrapper's own no-gate run the gain is 2.73×; the extra 0.10× to vanilla reproduces across sessions but is not computational (the no-gate output is pixel-identical to vanilla's) and has no established cause, so 2.73–3.02× is the honest range — see Benchmarks footnote ⁷. The COMPARISON.md portrait row is a separate v0.9.0 measurement (1.74× warm-median, 25 of 48 skipped, SSIM 0.987) taken before this change. On a 32 GB Mac, stock 4-bit quantization is grainy for Qwen — the showcase portraits use a mixed-precision build (see [docs/variants/qwen-image.md](docs/variants/qwen-image.md)); mlx-teacache itself stays quantization-agnostic. Reproduce with `uv run python scripts/bench_comparison.py --only qwen-image`.
...
| `qwen-image` (CFG)⁷ | 50 | 850.3s | 282.0s | **3.02×** | **33 / 50** | step-skipping (2.73× against its own no-gate run; see ⁷ on the gap) |
...
⁷ `qwen-image` under CFG at 50 steps, `guidance=4.0`, **768×768** (its pinned recipe — the rest of the table is 512×512), q4, per-variant default `rel_l1_thresh=0.30`. Measured 2026-09-01 under the subprocess-per-rep harness (mflux 0.18.0, v0.10.0 gate): **3.02× combined** against vanilla (850.3 s → gated 282.0 s medians), the largest gain in this table, and **2.73×** against the wrapper's own no-gate run (770.7 s → 282.0 s). Read the pair as a range, not a decomposition. The no-gate wrapper produced output pixel-identical to vanilla's — the same kernels over the same data, with nothing skipped — yet timed 1.10× faster, and the 0.9.0 sweep showed the same thing (zero-skip wrapper 1.18–1.20× ahead of vanilla), so the effect reproduces across sessions. It is not step-skipping, not a change in the computation, and not `mx.compile`-path avoidance (mflux does not wrap Qwen's `_predict`); what it is has not been established, so the true figure sits between 2.73× and 3.02×. 33 of the 48 active steps skipped in every rep (the table's `33 / 50` counts nominal steps), longest run of consecutive skips 4. This is a change from 0.9.x, which skipped 24 at the same threshold and recipe — the v0.10.0 anchoring lets more steps qualify, and SSIM against vanilla moves from 0.978 to 0.967, above the 0.95 floor the parity gate enforces. Peak memory 26.2 GB, the highest in the table and above this machine's 25.0 GB recommended working set, so the run completes only while host memory pressure stays low. Treat 32 GB as the floor and confirm swap is near empty before reproducing. Since 0.10.1 the bench worker bounds its cache pool and aborts once active plus cached MLX memory exceeds physical memory minus 4 GiB (28 GiB on this machine), so the command above no longer pages the machine; under that guard a plain-q4 run of this recipe on 2026-09-05 peaked at 26.2 GiB and completed (from that run's log, not a committed artifact). The figures in this footnote are 0.10.0's and stand as published; 0.10.1 changed the gate's arithmetic by at most 0.16 percent per step on this variant and did not re-measure it, so the fresh sweep and bench are the next benchmark run (see the CHANGELOG). Full report: `_artifacts/v0.10.0_bench_qwen_image.json`. Reproduce with `uv run python scripts/bench_speedup.py --variant qwen --three-way --reps 3 --report out.json`.
...
| M1 Max / Ultra, M2 Max / Ultra | compiled | **1.57× measured** on M1 Max FLUX.1-dev / 25 steps; 1.22× on klein-base-4b 50-step CFG; 1.37× on klein-base-9b 50-step CFG; 1.31× on z-image-base 50-step CFG |
