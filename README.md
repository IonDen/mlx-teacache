...
| `qwen-image` (CFG)⁷ | 50 | 850.3s | 282.0s | **3.02×** | **33 / 50** | step-skipping + a 1.10× wrapper gain of unidentified origin |
...
⁷ `qwen-image` under CFG at 50 steps, `guidance=4.0`, **768×768** (its pinned recipe — the rest of the table is 512×512), q4, per-variant default `rel_l1_thresh=0.30`. Measured 2026-09-01 under the subprocess-per-rep harness (mflux 0.18.0, v0.10.0 gate): **3.02× combined** (vanilla 850.3 s → gated 282.0 s medians), the largest gain in this table, splitting into 2.73× from step-skipping and 1.10× from the wrapper itself (no-gate median 770.7 s against vanilla's 850.3 s, with nothing skipped). That 1.10× is *not* `mx.compile`-path avoidance: mflux does not wrap Qwen's `_predict` in `mx.compile`. It reproduced across all three reps with no overlap between the two distributions, but its cause is not established, so it is reported on its own rather than folded into the step-skipping figure. 33 of the 48 active steps skipped in every rep (the table's `33 / 50` counts nominal steps), longest run of consecutive skips 4. This is a change from 0.9.x, which skipped 24 at the same threshold and recipe — the v0.10.0 anchoring lets more steps qualify, and SSIM against vanilla moves from 0.978 to 0.967, above the 0.95 floor the parity gate enforces. Peak memory 26.2 GB, the highest in the table; this recipe wants real headroom on a 32 GB machine. Full report: `_artifacts/v0.10.0_bench_qwen_image.json`. Reproduce with `uv run python scripts/bench_speedup.py --variant qwen --three-way --reps 3 --report out.json`.
