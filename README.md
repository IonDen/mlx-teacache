...
**[`qwen-image`](docs/variants/qwen-image.md)** — Qwen-Image base (Alibaba, Apache-2.0), a ~20B dual-stream MMDiT and the first variant that proxies `flux.transformer` (the FLUX.1 pattern) *and* runs true two-pass CFG. It's FLUX-shaped, so the gate taps the FLUX-canonical modulated block-0 input — calibrated in-repo at R² 0.849, well above Z-Image's 0.400 and the FLUX.2 family's 0.11–0.47. Per-variant default `rel_l1_thresh=0.30`, set from the threshold sweep. The v0.11.0 three-way bench at the red-apple 768×768 recipe (subprocess-per-rep, three cold reps, q4, 50 steps, g=4.0, text encoders freed after prompt encoding): **2.68× combined** (642.6 s → 239.4 s), 33 of 48 active steps skipped, SSIM 0.967 vs vanilla. Qwen moved further under the corrected gate than any other variant — 0.9.x skipped 24 at this same threshold and held SSIM 0.978 — because its accumulator sits nearest the threshold; the threshold sweep on the variant page shows the whole curve, and `rel_l1_thresh=0.20` gives the 0.9.x quality point back. Against the wrapper's own no-gate run the gain is 2.57×; the no-gate wrapper's 1.04× over vanilla sits inside the run-to-run spread — see Benchmarks footnote ⁷. Every Qwen number here is for `Qwen/Qwen-Image` on mflux 0.18. mflux 0.19 resolves the same alias to `Qwen/Qwen-Image-2512`, a newer checkpoint the coefficients were not calibrated on; the variant still applies there, `apply_teacache` raises `TeaCacheUncalibratedCheckpointWarning` once, and the honest status of that checkpoint is unverified until it is calibrated with `scripts/calibrate_qwen.py`. The COMPARISON.md portrait row is a separate v0.9.0 measurement (1.74× warm-median, 25 of 48 skipped, SSIM 0.987) taken before this change. On a 32 GB Mac, stock 4-bit quantization is grainy for Qwen — the showcase portraits use a mixed-precision build (see [docs/variants/qwen-image.md](docs/variants/qwen-image.md)); mlx-teacache itself stays quantization-agnostic. Reproduce with `uv run python scripts/bench_comparison.py --only qwen-image`.
