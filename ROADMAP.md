...
- **v0.11.1** — mflux 0.20 support. The pin widens to `>=0.17.5,<0.21` after the real-weights parity lane passed on 0.20.0 for FLUX.1, Krea, the FLUX.2 Klein family and Z-Image. The upstream changes since 0.19.1 (optional gradient checkpointing in the FLUX.2 and Z-Image forwards, a tiling setting on FLUX.2's post-loop decode) sit outside the path the copied forwards take. Qwen-Image is unchanged from 0.11.0 (Qwen-Image-2512 still warns); mflux 0.20's Qwen-Image-2.1 is not a supported variant. No change to the API, the gate or any generated image.
- **v0.11.0** — mflux 0.19 support and FLUX.1 Krea [dev]. The pin widens to `>=0.17.5,<0.20` after the real-weights parity lane passed on 0.19.1 for FLUX.1, Krea, the FLUX.2 Klein family and Z-Image (mflux 0.19 needs MLX 0.32 and torch 2.13; pair it with mlx-taef 0.8.1 or later). On 0.19 the `qwen-image` alias loads Qwen/Qwen-Image-2512, which the Qwen coefficients were not calibrated on; `apply_teacache` warns there until that checkpoint is calibrated. Krea ships its own calibration (FLUX.1-dev's tuple scores R² −496 on it), a default threshold of 0.30 at a sharp sweep knee, and a 1.62× three-way bench. Qwen-Image's default was re-swept under the current gate and stays at 0.30; re-benched with the text encoders freed at 2.68× combined, 2.57× from gating, peak 24.1 GB. Weight-free tests pin the private mflux surface and fingerprint the copied forwards per mflux version.
- **v0.10.1** — Safety patch. The heavy scripts bound MLX's cache pool and abort themselves above physical memory minus 4 GiB instead of paging the machine; the library materialises its cached residual and releases it after each generation and on restore; the gate's reductions return float32 scalars and a non-finite prediction counts as a miss; FLUX.2 and Z-Image roll back a failed callback registration. FLUX.1-dev and Z-Image re-validated unchanged on real weights. The Qwen-Image re-sweep and re-bench are deferred to the next benchmark run.
- **v0.10.0** — Gate-correctness release. The gate now anchors its relative-L1 signal on the immediately previous step (computed or skipped), the quantity the calibration scripts and upstream TeaCache measure, and a runaway guard forces a recompute after 8 consecutive skips. FLUX.1, the FLUX.2 Klein family and Z-Image do not move at their shipped defaults — skip counts and per-step patterns re-measured identical to 0.9.x, showcase images byte-identical — while Qwen-Image does: it skips 33 of 48 active steps where 0.9.x skipped 24, trading SSIM 0.978 for 0.967 against vanilla in exchange for a 3.02× speedup, and its threshold is due a fresh sweep. The real-weights parity lane passed with no quality failures. Distilled Klein variants warn at apply time; Z-Image gains a default-threshold SSIM gate; the bench scripts persist per-chunk results and resume; all five engaged variants re-benched three-way (FLUX.1-dev 1.57× median / 1.50× fastest-to-fastest, klein-base-4b 1.22×, klein-base-9b 1.37×, Z-Image 1.31×, Qwen-Image 3.02×).
- **v0.9.3** — Python 3.10 support: `requires-python` widened to `>=3.10`, matching mlx-taef v0.7.1. No library code changed — the only 3.11-isms were in test and bench tooling (`tomllib` imports now guarded with `tomli`; `datetime.UTC` replaced with `timezone.utc`). CI tests Python 3.10–3.13, and a configuration-guard suite keeps the floor, classifiers, CI matrix, checker targets, and README badge on one supported-versions list. Gate math, coefficients, and generated output are unchanged.
