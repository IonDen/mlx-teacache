...
- **v0.9.0** — `qwen-image` support: Alibaba's ~20B dual-stream MMDiT (Apache-2.0). The first variant that proxies `flux.transformer` (the FLUX.1 pattern) and also runs true two-pass CFG — `generate_image` calls the transformer twice per step and combines outside it, so a branch-pairing state machine shares one gate decision across the pair and keeps two cached residuals. Qwen is FLUX-shaped, so the FLUX-canonical modulated-input gate calibrates at R² 0.946 (vs Z-Image's 0.40). Per-variant default `rel_l1_thresh=0.25`; **1.41× warm-median** at the shared 512² portrait recipe (6 of 18 active steps skipped, SSIM 0.99) — the largest speedup of the comparison set, entirely step-skipping (Qwen has no `mx.compile` path to bypass). The 512² recipe is a memory fallback from 768²: the 20B q4 model peaks ~27.6 GB on a 32 GB M1 Max regardless of resolution.
