...
| MLX peak: encoders loaded / prompt encoded + model loaded / generation | 3.3 GiB / 9.5 GiB / 14.7 GiB | 3.3 GiB / 9.5 GiB / 15.4 GiB |
...
Recipe: 25 steps, guidance 3.5, q4, 768×1024, seed 42. Checkpoint `black-forest-labs/FLUX.1-dev`; preview decoder `taef1`. mlx-teacache 0.11.1 (harness at commit `bcaac6f`), mflux 0.20.0, MLX 0.32.2, mlx-taef 0.8.3. Multi-run measurement of this model: [bench report](../../_artifacts/v0.10.0_bench_flux1_dev.json).
...
```bash
uv run --no-sync python scripts/bench_comparison.py --only flux1-dev --max-workers 1
uv run --no-sync python scripts/bench_comparison.py --only flux1-dev --max-workers 1
uv run --no-sync python scripts/bench_comparison.py --only flux1-dev --finalize --export-jpg
