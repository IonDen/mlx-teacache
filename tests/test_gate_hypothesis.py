...
_FINITE_FLOATS = st.floats(min_value=-1e3, max_value=1e3,
...
@given(step_idx=st.integers(min_value=0, max_value=99),
...
def test_forced_windows_honored(step_idx, skip_first, skip_last):
    ...
    dec = gate_step(state, rel_l1_thresh=0.5, coefficients=(0.0,)*4 + (1.0,),
