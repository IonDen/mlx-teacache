"""flux2-klein-4b variant detection + config."""
...
def test_coefficients_match_v05_registry() -> None:
    from mlx_teacache.coefficients import _FLUX2_KLEIN_4B_COEFFS
    from mlx_teacache.variants.flux2_klein_4b.config import COEFFICIENTS
    assert COEFFICIENTS == _FLUX2_KLEIN_4B_COEFFS


def test_default_thresh_is_none() -> None:
    """Distilled 8-step gate doesn't engage; package fallback used."""
    from mlx_teacache.variants.flux2_klein_4b.config import DEFAULT_THRESH
    assert DEFAULT_THRESH is None


def test_matches_alias() -> None:
    from mlx_teacache.variants.flux2_klein_4b.detect import matches
    assert matches(_FakeFlux2(["flux2-klein-4b"])) is True
